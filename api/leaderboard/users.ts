...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      // 解析查询参数
      const sortBy = (req.query.sortBy as string) || 'posts_count'
      const timeRange = (req.query.timeRange as string) || 'all'
      const limit = parseInt((req.query.limit as string) || '20')
      
      // 验证参数
      const validSortBy = ['posts_count', 'likes_count', 'views']
      const validTimeRange = ['day', 'week', 'month', 'all']
      
      if (!validSortBy.includes(sortBy)) {
        return sendErrorResponse(res, API_ERRORS.INVALID_PARAMS, {
          message: '无效的排序方式，有效值：posts_count, likes_count, views'
        })
      }
      ...
      // 获取排行榜数据
      const users = await leaderboardDB.getUsersLeaderboard({
        sortBy,
        timeRange,
        limit
      })
      
      console.log('获取用户排行榜成功:', { sortBy, timeRange, limit, count: users.length })
